  ...
  async (
    ...
    const cachedFetch = unstable_cache(
      async (token: string) => {
        ...
        const individuals: IndividualRankRow[] = (
      ...
      ["leaderboard", user.church_id, period],
      {
        tags: [tag.leaderboard(user.church_id)],
...
export const getMyTeam = withPermission(
  ...
  > => {
    ...
    const cachedFetch = unstable_cache(
      ...
      ["my-team", user.church_id, user.email],
      { tags: [tag.myTeam(user.church_id)], revalidate: CACHE_TTL.dashboard }
    );
